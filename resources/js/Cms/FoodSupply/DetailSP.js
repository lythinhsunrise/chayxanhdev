...
const DetailSP = () => {
  ...
  const { getListQtyFoods, getListMenus, getListUsers, updateOrder, getFoodSPByID, user, getListStores, updateFoodSP, updateAcceptFoodSP } = useContext(AppContext);
  ...
  const operations = <>
    {user.store_id === item.id_store_share ? <Button onClick={() => onAccept()} type="primary" style={{ marginRight: 8 }}>
      Chấp thuận
    </Button> : null}
  ...
  useEffect(() => {
    ...
    getFoodSPByID(params.id).then((res) => {
      setItem(res.data.data)
      let orderItems = JSON.parse(res.data.data.orderD);
      setOrderD(orderItems)
      setNotes(res.data.data.notes)
      getListMenus().then((response) => {
        // setItemMenu(response.data.data)
        let temp = response.data.data
        getListQtyFoods(res.data.data.id_store_share).then((res) => {
          temp.map((item, index) => {
            res.data.data.map((i) => {
  ...
  const onSubmit = () => {
    form.validateFields().then((values) => {
      ...
      values = { ...values, orderD, id: params.id, notes, status: 0 }
      // setLoadingForm(true)
      console.log(values)
      updateFoodSP(values).then((res) => {
        setLoadingForm(false)
  ...
  const onAccept = () => {
    form.validateFields().then((values) => {
      if (orderD.length === 0) {
        openNotification({ status: false, message: 'Không thể chấp nhận yêu cầu, chưa có món nào!' });
        return;
      }
      values = { ...values, orderD, id: params.id, notes, status: 1, id_store_share: item.id_store_share, id_store_get: item.id_store_get }
      // setLoadingForm(true)
      // console.log(values)
      updateAcceptFoodSP(values).then((res) => {
        setLoadingForm(false)
        openNotification(res.data);
        navigate("/admin/foods_supply")
      })
    ...
    <>
      ...
      <div className="site-layout-background" style={{ padding: 16, minHeight: 480 }}>
        <Card
          size="small"
          title={
            item.name ? <h3>{item.name}<br /><small>{item.username}</small></h3> : <h3>{'Yêu cầu chuyển món ID ' + item.id}</h3>
          }
          style={{ width: '100%' }}
          extra={item.status ? <Button onClick={() => navigate("/admin/foods_supply")} icon={<RollbackOutlined />}>
            Quay lại
          </Button> : operations}
        >
            ...
            >
              <Row>
                ...
                <Col xs={24} xl={12}>
                  ...
                  {(item.status === 1 && params.id) ? <h3 style={{ color: '#307839', border: '2px solid #307839', width: '150px', paddingLeft: '8px', marginTop: '8px' }}>Đã chấp thuận</h3> : <h3 style={{ color: '#AA336A', border: '2px solid #AA336A', width: '150px', paddingLeft: '8px', marginTop: '8px' }}>Yêu cầu mới</h3>}
                </Col>
              </Row>
              <br />

              {item.status ? null : <Button type="primary" onClick={showDrawer}>
                Chọn món
              </Button>}
              <Drawer
                title="Món ăn"
